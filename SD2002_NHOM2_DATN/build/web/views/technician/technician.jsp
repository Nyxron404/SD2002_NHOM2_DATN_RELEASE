...
<body>
    ...
    <%-- SCRIPT XỬ LÝ TÌM KIẾM BẰNG JS VÀ SỰ KIỆN CLICK OUTSIDE --%>
    <script>
        ...
        // TỰ ĐỘNG ĐÓNG BẤM CẢ CÁC MODAL KHI CLICK RA NGOÀI VÙNG TỐI
        window.onclick = function(event) {
            if (event.target.classList.contains('modal-overlay')) {
                // 1. Đóng Modal Tạo mới
                let createToggle = document.getElementById('createModalToggle');
                if (createToggle) createToggle.checked = false;

                // 2. Đóng tất cả Modal Sửa
                let editToggles = document.querySelectorAll('input[id^="editToggle-"]');
                editToggles.forEach(toggle => toggle.checked = false);
